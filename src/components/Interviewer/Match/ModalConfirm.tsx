...
const updateHour = (pastInterviewData: interviewData, newStartingHour: hour) => {
...
const ModalConfirm = (props: { 
  ...
  const { setShowConfirm, setNewInterviewData, pastInterviewData, availableHours } = props
    ...
    >
      <Box width="medium" gap="medium" background="background-contrast" pad="medium">
        ...
        <Button label="Ok" size="small" margin="medium" type="submit" onClick={() => {
          // @ts-ignore
          console.log(`New Interview Scheduled with ${newData.uid} at ${newData.interviewDay.toString()} ${newData.interviewHour}`)
